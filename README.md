...
For Cursor **project-level** MCP, open the target project folder in Cursor and use an Agent chat **inside that project window**. Agents started from Cursor **Home / global** context may not load project MCP servers.  
中文：使用项目级 MCP 时，请先打开目标项目，并在**该项目窗口内**使用 Agent；从主页或全局入口打开的 Agent **可能**无法加载项目级 MCP。
